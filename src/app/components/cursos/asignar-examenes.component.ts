...
export class AsignarExamenesComponent implements OnInit {
  ...
  ngOnInit(){
      ...
      //como recibe texto hay que cambiarlo, por eso el pipe
      this.autocompleteControl.valueChanges.pipe(
        map(valor => typeof valor === 'string' ? valor : valor.nombre),
        mergeMap(valor => valor ? this.examenService.filtrarPorNombre(valor): []) //cambiar de observable a observable usamos flatmap
      ).subscribe(examenes => this.examenesFiltrados = examenes);
    }
//mergeMap es lo nuevo, ya que flatMap esta deprecated
  ...
  seleccionarExamen(event: MatAutocompleteSelectedEvent): void{
    ...
    if(!this.existe(examen.id)){
      ...
      console.log(this.examenesAsignar);
        //las ultimas 3 resetean el autocomplete, sin valor
    } else{
      Swal.fire('Error',
  ...
  asignar(): void{
    console.log(this.examenesAsignar); //opcional solo para ver
    ...
    .subscribe(curso => {
      ...
      Swal.fire(
        'Asignados:',
        `Exámenes asignados con éxito al curso ${curso.nombre}`,
        'success'
      );
      this.tabIndex = 2;
    })
