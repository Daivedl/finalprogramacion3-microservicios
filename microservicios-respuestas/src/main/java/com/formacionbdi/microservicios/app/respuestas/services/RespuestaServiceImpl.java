...
import com.formacionbdi.microservicios.app.respuestas.models.respository.RespuestaRepository;
...
public class RespuestaServiceImpl implements RespuestaService {
	...
	public Iterable<Respuesta> findRespuestaByAlumnoByExamen(Long alumnoId, Long examenId) {
		...
		List<Long> preguntasIds = preguntas
				.stream().map(p -> p.getId())
				.collect(Collectors.toList());//convierte 2 veces
		...
		//al tenerlo de forma local, podemos hacerlo así
	...
	public Iterable<Long> findExamenesIdsConRespuestasByAlumno(Long alumnoId) {
		...
		if(respuestasAlumno.size() > 0) {
			List<Long> preguntasIds = respuestasAlumno.stream()
					.map(r->r.getPreguntaId())
					.collect(Collectors.toList());
			examenIds = examenClient.obtenerExamenesIdsPorPreguntasIdsRespondidas(preguntasIds);
		}*/
