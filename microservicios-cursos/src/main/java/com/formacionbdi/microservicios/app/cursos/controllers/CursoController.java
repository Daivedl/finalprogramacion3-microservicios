...
public class CursoController extends CommonController<Curso, CursoService> {
	...
	public ResponseEntity<?> listar(Pageable pageable) {
		...
		// lista completo completo la informacion de los alumnos
	...
	public ResponseEntity<?> listar() {
		List<Curso> cursos = ((List<Curso>) service.findAll()).stream().map(c -> {
			c.getCursoAlumnos().forEach(ca -> {
				...
				alumno.setId(ca.getAlumnoId());
				c.addAlumno(alumno);
			}); // agrega los alumnos donde deberian ir en vez de donde estaban antes
			return c;
	...
	public ResponseEntity<?> eliminarAlumnos(@RequestBody Alumno alumno, @PathVariable Long id) {
		...
		dbCurso.removeCursoAlumno(cursoAlumno); // mas simple ya que por el id se elimina
		return ResponseEntity.status(HttpStatus.CREATED).body(this.service.save(dbCurso));
	}
	...
	public ResponseEntity<?> eliminarExamen(@RequestBody Examen examen, @PathVariable Long id) {
		...
		dbCurso.removeExamen(examen); // mas simple ya que por el id se elimina
		return ResponseEntity.status(HttpStatus.CREATED).body(this.service.save(dbCurso));
	}
